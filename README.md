...
public class BallManager {
	...
	private LinkedList<IBall> balls = new LinkedList<>();
	...
	public void checkCollision(Target target) {
		...
		if (!removing) {
			try {
				...
				new java.util.Timer().schedule(timeout(() -> balls.forEach(ball -> targetCollisions(target, ball))),
						20);
			}
		}
	}
		...
		if (ball.checkCollision(target)) {
			switch (target.getClass().getName()) {
				...
				case "DuplicatorTarget":
					balls.addLast(createBallInstance(target.getCx(), target.getCy(), 20, 20, Color.RED, 0.65,
							ball.getVx(), ball.getVy()));
					CompletableFuture.runAsync(() -> new java.util.Timer().schedule(timeout(() -> cleanup()),
